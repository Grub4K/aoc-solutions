...
def run(moves):
    yield sum(
        (own_hand + 1) + (get_result(own_hand, other_hand) * 3) for other_hand, own_hand in moves
    )

    yield sum((get_own_hand(other_hand, result) + 1) + (result * 3) for other_hand, result in moves)
